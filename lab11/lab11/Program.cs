...
namespace lab11
{
    class Program
    {
        ...
        {
            StreamReader f_in = new StreamReader("Data.txt");
#if !DEBUG
            ...
            for (int i = 0; i<all.Count; i++)
            {
                ...
                {
                    k += 1;
                    counts+= Convert.ToDouble(all[i].Balance);
                }
            ...
            for (int i = 0; i < all.Count; i++)
            {
                if (all[i].Balance > maxbalance)
                {
                    maxbalance = all[i].Balance;
